...
from scijo.integrate.utility import QuadResult, kronrod_nodes, kronrod_weights, gauss_weights

fn _gauss_kronrod_21[
    dtype: DType,
    integrand: fn (x: Scalar[dtype], args: Optional[List[Scalar[dtype]]]) -> Scalar[
        dtype
    ],
](a: Scalar[dtype], b: Scalar[dtype], args: Optional[List[Scalar[dtype]]]) -> Tuple[
    ...
    for i in range(1, 11):
        ...
        var x1 = center - half_length * node  # Left point: (a+b)/2 - (b-a)/2 * node
        var x2 = center + half_length * node  # Right point: (a+b)/2 + (b-a)/2 * node

        var f1 = integrand(x1, args)
        var f2 = integrand(x2, args)
        var fsum = f1 + f2
...
fn quad[
    ...
    if a == b:
        # return (Scalar[dtype](0), Scalar[dtype](0), "Success")
        return QuadResult(
    ...
    # Initial evaluation on entire interval
    var initial_result = _gauss_kronrod_21[dtype, integrand](lower, upper, args)
    var result_k = initial_result[0]
    var _ = initial_result[1]  # Gauss result - not used for now
    var neval = initial_result[2]

    # For now, use a conservative error estimate since Gauss weights may be wrong
    # In a proper implementation, this would be abs(result_k - result_g)
    var error_est = abs(result_k) * 1e-12  # Conservative estimate

    # Check if we already meet tolerance
    ...
    # Use a simple iterative subdivision approach
    var current_result = result_k
    var current_error = error_est
    var subdivisions = 0

    # Simple adaptive algorithm - bisect interval with largest error
    ...
    if current_error <= tolerance:
        # return (sign * current_result, current_error, "Success")
        return QuadResult(
        ...
        return QuadResult(
            ...
            abserr=current_error,
            neval=neval,
            ier=1,  # Maximum subdivisions reached
            last=subdivisions,
        )
